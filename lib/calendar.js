...
Calendar = (function(_super) {
  ...
  function Calendar() {
    this._bindDrag = __bind(this._bindDrag, this);
  ...
  Calendar.prototype._tpl = {
    ...
    event: '<div class="event" draggable="true">\n  <div class="event-wrapper">\n    <p class="content"></p>\n  </div>\n</div>',
  ...
  Calendar.prototype._bindDrag = function() {
    this.el.on('dragstart.calendar', '.event', (function(_this) {
      return function(e) {
        var $copy, $event, days, event;
        $event = $(e.currentTarget);
        event = $event.data('event');
        console.log(event);
        e.originalEvent.dataTransfer.setData('Text', event.id);
        ...
        $event.css({
          'width': 'auto',
          'min-width': _this.el.find('.day').eq(0).width() + 'px'
        });
        ...
        setTimeout(function() {
          ...
          return _this.el.find(".event[data-id=" + event.id + "]").addClass('dragged');
        }, 0);
        return _this.trigger('eventdragstart', [event]);
      };
    })(this));
    this.el.on('dragend.calendar', '.event', (function(_this) {
      return function(e) {
        return console.log('drag end event emit');
      };
    })(this));
    this.el.on('dragenter.calendar', '.day', (function(_this) {
      return function(e) {
        var $target, days, event, index;
        e.preventDefault();
        event = _this.dragging;
        if (!event) {
          return;
        }
        days = event.end.clone().startOf('day').diff(event.start.clone().startOf('day'), 'd');
    ...
    this.el.on('dragover.calendar', '.day', (function(_this) {
      return function(e) {
        e.originalEvent.dataTransfer.dropEffect = 'move';
        return e.preventDefault();
      };
    })(this));
    return this.el.on('drop.calendar', '.day', (function(_this) {
      return function(e) {
        var $event, $target, differ, event, newDate;
        e.preventDefault();
        event = _this.dragging;
        if (!event) {
          return;
